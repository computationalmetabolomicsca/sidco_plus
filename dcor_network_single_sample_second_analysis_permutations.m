...
%[net1] = dcor_network_single_sample_second_analysis("C:\Project\CorrSize_Inflammation\test1.csv", "C:\Project\CorrSize_Inflammation\",0.05, 0.1, 1, 1,15,50,100);

function net1 = dcor_network_single_sample_second_analysis(Filename, outputdirectory, alpha, ci_min, Normalize, Imputer,select_label,NSubset,NBoot, do_plot, layout, seed, B_perm)
...
if nargin < 7  || isempty(select_label),   select_label   = 1;  end
if nargin < 8  || isempty(NSubset),   NSubset   = 20;  end
if nargin < 9  || isempty(NBoot),    NBoot    = 20; end
if nargin < 10  || isempty(do_plot),   do_plot   = true;  end
if nargin < 11  || isempty(layout),    layout    = 'force'; end
if nargin < 12  || isempty(seed),      seed      = 1;     end
if nargin < 13 || isempty(B_perm),    B_perm    = 100;   end
...
figN = plot_networks(net1,  threshold_types, threshold_labels, ...
    fig_colors, pm);
save_figure(figN, out_dir, 'figN_networks', net1);
