...
%[net1] = dcor_network_single_sample_first_analysis_permutations("C:\Project\Complimet\SIDCOPLUS\SingleSampleFirstAnalysis\test1.csv",...
%   "C:\Project\Complimet\SIDCOPLUS\SingleSampleFirstAnalysis", 0.05, 0.1, 1, 1,10,10);



function net1 = dcor_network_single_sample_first_analysis_permutations(Filename,outputdirectory,...
    alpha, ci_min, Normalize, Imputer,NSubset,NBoot, do_plot, layout, seed, B_perm)
...
if nargin < 4  || isempty(ci_min),    ci_min    = 0.10;  end
if nargin < 5  || isempty(Normalize), Normalize = 1;     end
if nargin < 6  || isempty(Imputer),   Imputer   = 1;     end
if nargin < 7  || isempty(NSubset),   NSubset   = 20;  end
if nargin < 8  || isempty(NBoot),    NBoot    = 20; end
if nargin < 9  || isempty(do_plot),   do_plot   = true;  end
if nargin < 10  || isempty(layout),    layout    = 'force'; end
if nargin < 11 || isempty(seed),      seed      = 1;     end
if nargin < 12 || isempty(B_perm),    B_perm    = 100;   end
...
fig1 = plot_heatmaps(net1,pm);
save_figure(fig1, out_dir, 'fig1_dcor_heatmaps', net1);

fig3 = plot_degree_comparison(net1, threshold_types, threshold_labels, fig_colors,pm);
save_figure(fig3, out_dir, 'fig3_degree_comparison', net1);

fig4 = plot_bc_comparison(net1, threshold_types, threshold_labels, fig_colors,pm);
...
axis(ax1,'square');
% only show correction with both qval and ci
...
%    'XTickLabelRotation',90,'FontSize',6,'TickLength',[0 0]);
%title(ax2,[Filename, ' —  dCor (FDR sig.)'],'FontSize',9,'FontWeight','bold');
%axis(ax2,'square');
...
%    'XTickLabelRotation',90,'FontSize',6,'TickLength',[0 0]);
%title(ax3,[Filename, '  —  dCor Bernstein CI pass'],'FontSize',9,'FontWeight','bold');
%axis(ax3,'square');
...
% =========================================================================
function fig = plot_degree_comparison(net1, thresh_types, thresh_labels, fig_colors,pm)
...
fig = figure('Name','Degree Information','NumberTitle','off', ...
    'Position',[50 50 1600 900],'Color',[0.97 0.97 0.97],'Visible', 'on');
...
% =========================================================================
function fig = plot_bc_comparison(net1, thresh_types, thresh_labels, fig_colors,pm)
...
fig = figure('Name','Betweenness Centrality','NumberTitle','off', ...
    'Position',[60 60 1600 900],'Color',[0.97 0.97 0.97],'Visible', 'on');
