...
% =========================================================================================================================
% SINGLE SAMPLE ANALYSIS OF UNBIASED DISTANCE CORRELATION - network
...
%select_label="Lac"; %metabolite selected to show network
%Nsubsample=50;
%Npermutations=20;
%[net1] =...
%dcor_network_single_sample_first_analysis_permutations("Inut_File.csv","outputdirectory", alpha, minCI, Normalization_box, Imputationa_box,select_label,Nsubsample,Npermutations);

function net1 = dcor_network_single_sample_second_analysis_permutations(Filename, outputdirectory, alpha, ci_min, Normalize, Imputer,select_label,NSubset,NBoot)
...
[figN,subnodes] = plot_networks(net1,  threshold_types, threshold_labels, ...
    fig_colors, pm);
save_figure(figN, out_dir, 'figN_networks', net1,subnodes);
...
% =========================================================================
function [fig,subnodes] = plot_networks(net1,  thresh_types, thresh_labels, ...
        fig_colors, pm)
...
fig = figure('Name','Network Graphs','NumberTitle','off', ...
    'Position',[40 40 1800 700],'Color',[0.97 0.97 0.97],'Visible', 'off');
...
% only plot selection by qval and CI
    for t = n_thresh:n_thresh
        ...
        if isempty(ri)
            ...
            axis(ax,'off');
            title(ax, sprintf('%s  %s', set_names, thresh_labels{t}), ...
                'FontSize',8,'FontWeight','bold');
            continue;
        end
        % pm.select_label is now a label string present in net.labels.
        % Find its index (if multiple matches, take the first).
        sel_label = pm.select_label;
        if ischar(sel_label) || isstring(sel_label)
            sel_label = char(sel_label);
            k = find(strcmp(net.labels, sel_label), 1);
            if isempty(k)
                warning('Selected label "%s" not found in network labels. Falling back to node 1.', pm.select_label);
                k = 1;
            end
        else
            % If a numeric index was provided, use it (with bounds check)
            k = round(sel_label);
            if isempty(k) || ~isnumeric(k) || k < 1 || k > p
                warning('Invalid select_label provided. Falling back to node 1.');
                k = 1;
            end
        end
        ...
        for ni = 1:numel(subnodes)
            mk_sz = 40 + 100*(deg(ni)/max_deg);
            scatter(ax, pos(ni,1), pos(ni,2), mk_sz, fig_colors.(thresh_types{t}), ...
                'filled','MarkerEdgeColor',[1 1 1],'LineWidth',0.8);
            text(ax, pos(ni,1), pos(ni,2) + 0.08, lbl{subnodes(ni)}, ...
...
%  SAVE_FIGURE
% =========================================================================
function save_figure(fig, out_dir, base_name, net1,subnodes)
...
try
    ...
    s = struct();
  sub = subnodes(:);  % ensure column vector

    s.n_features        = numel(sub);
    s.labels            = net1.labels(sub);
    s.net1.n             = net1.n;
    s.net1.alpha         = net1.alpha;
    s.net1.ci_min        = net1.ci_min;
%    s.net1.degree_full  = net1.degree_full(sub)';
%    s.net1.degree_fdr   = net1.degree_fdr(sub)';
%    s.net1.degree_ci    = net1.degree_ci(sub)';
    s.net1.degree_fdr_ci= net1.degree_fdr_ci(sub)';
%    s.net1.bc_full      = net1.bc_full(sub)';
%    s.net1.bc_fdr       = net1.bc_fdr(sub)';
%    s.net1.bc_ci        = net1.bc_ci(sub)';
%    s.net1.bc_fdr_ci    = net1.bc_fdr_ci(sub)';
    s.net1.dcor         = mat2rows(net1.dcor(sub, sub));
%    s.net1.pval         = mat2rows(net1.pval(sub, sub));
%    s.net1.qval         = mat2rows(net1.qval(sub, sub));
%    s.net1.ci_lo        = mat2rows(net1.ci_lo(sub, sub));
%    s.net1.ci_hi        = mat2rows(net1.ci_hi(sub, sub));
%    s.net1.adj_full     = mat2rows(net1.adj_full(sub, sub));
%    s.net1.adj_fdr      = mat2rows(net1.adj_fdr(sub, sub));
%    s.net1.adj_ci       = mat2rows(net1.adj_ci(sub, sub));
%    s.net1.adj_fdr_ci   = mat2rows(net1.adj_fdr_ci(sub, sub));
